        ...
        <div class="vg-wrap vg-element vg-full gallery_container">
            ...
            <?php if ($this->data[$row]) : ?>
                ...
                <?php foreach ($this->data[$row] as $item) : ?>
                    <a href="<?=$this->adminPath . 'delete/' . $this->table . '/' . $this->data[$this->columns['id_row']] . '/' . $row . '/' . base64_encode($item)?>"
                ...
                <?php
                    for ($i = 0; $i < 2; $i++) {
                        echo '<div class="vg-dotted-square vg-center empty_container" draggable="true"></div>';
                    }
                ?>
            <?php else : ?>
                <?php
