...
abstract class BaseAdmin extends BaseController
{
    ...
    protected function outputData(){
        ...
        $this->header = $this->render(ADMIN_TEMPLATE . 'include/header');            // 14-40
    ...
    protected function sendNoCacheHeaders(){
        ...
        header("Cashe-Control: no-cashe, must-revalidate");
        header("Cashe-Control: max-age=0");
        header("Cashe-Control: post-check=0, pre-check=0");
    }
    ...
    protected function createTableData($settings = false){
        ...
        if(!$this->columns) new RouteException('Не найдены поля в таблице БД - ' . $this->table, 2);
    ...
    protected function createAlias($id = false){
        ...
        if($this->columns['alias']){
            ...
            $res_alias = $res_alias = $this->model->get($this->table, [
                'fields' => 'alias',
                'where' => $where,
                'operand' => $operand,
                'limit' => '1',
            ...
            /* $editController = new EditController(); // может быть так?
            if($_POST['alias'] && $id){
                method_exists($editController, 'checkOldAlias') && $editController->checkOldAlias($id);
            } */
                                ...
                                if (isset($item['join'][$tables[$otherKey]]) && $item['join'][$tables[$otherKey]]){
                                    ...
                                    foreach ($item['join'][$tables[$otherKey]] as $value){
                                        ...
                                        if (in_array($value['id'], $foreign))
                                            $this->data[$tables[$otherKey]][$key]['id'] = $value['id'];
                                    }
                                }
                            }
                        }
                        //exit;   //Выпуск №83 42-18 закоммент 619-724; Выпуск №84 8-27 закоммент 711-724
                    }else{
