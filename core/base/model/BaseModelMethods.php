      ...
      if(is_array($set['where']) && !empty($set['where'])){
         ...
         foreach($set['where'] as $key => $item) {
            ...
            if($operand === 'IN' || $operand === 'NOT IN') {
               ...
               $like_temlate = explode('%', $operand);

               foreach ($like_temlate as $lt_key => $lt){
                  if(!$lt) {
                     if(!$lt_key) {
                        $item = '%' . $item;
                     }else{
...
/*Интернет магазин с нуля на php Выпуск №44 11-38*/
   protected function createInsert($fields, $files, $except){
      ...
      $array_type = array_keys($fields)[0];   //проверил 79 21-47
